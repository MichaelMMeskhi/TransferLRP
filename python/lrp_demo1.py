...
nn = model_io.read('../mnist_mlp-Target.txt') # 99.16% prediction accuracy
nn.drop_softmax_output_layer() #drop softnax output layer for analyses


Xtest_old = data_io.read('../data/MNIST/test_images.npy')
Ytest_old = data_io.read('../data/MNIST/test_labels.npy')

Xtest = []
Ytest = []

for i in range(len(Ytest_old)):
    if Ytest_old[i] in [[5], [6], [7], [8], [9]]:
        Ytest.append(Ytest_old[i])
        Xtest.append(Xtest_old[i])
...
for i in I[:10]:
    ...
    R = nn.lrp(Rinit,'simple',0.01)    #as Eq(58) from DOI: 10.1371/journal.pone.0130140
    ...
    digit_hm = render.save_image([digit,hm],'../Target/heatmap_target'+str(i)+'.png')
    # data_io.write(R,'../Target/heatmap_target'+str(i)+'.npy')
