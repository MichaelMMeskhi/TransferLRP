...
class Linear:
	...
	def __init__(self, name=''):
		# Refresh layer to retrain with new W and B
		if name == None:
			self.W = numpy.random.randn(300, 100)
			self.B = numpy.random.randn(300,)
		else:
			# Load weights and biases from pretrained model
...
class Convolution:
	...
	def __init__(self,name):
		wshape = tuple(map(int,list(name.split("-")[-1].split("x"))))
